...
data_dir = 'exp_traj_obs_15_ep_len_77_goal_size_50'
images_dir = 'gym_pcgrl/envs/probs/zelda/{}.png'
persist_data_root_dir = 'cnn_data/{}/{}.png'
...
def infer(game, representation, model_path, **kwargs):
    ...
    if not non_random_start:
        ...
        agent = keras.models.load_model('/Users/matt/gym_pcgrl/gym-pcgil/gym_pcgil/cnn_obs_15_ep_len_77_goal_size_50.h5')
        # agent = keras.models.load_model(
        #     f'/Users/matt/gym_pcgrl/gym-pcgil/gym_pcgil/models/narrow_greedy_obs_size_{obs_size}_ep_len_{ep_len}.h5')
        # agent = keras.models.load_model('narrow_greedy_obs_size_15_ep_len_30_goal_map_size_50.h5')
        ...
        for i in range(kwargs.get('trials', 1)):
            ...
            while not dones:
                ...
                if mode == 'mlp':
                    ...
                    for row in range(15):
                        for col in range(15):
                            tile_img = images_dir.format(tile_map[list(X[row][col]).index(1)])
                            curr_level.append(tile_img)
                    curr_level = np.array(curr_level).reshape((15, 15))
                    images = []
                    for row in range(len(curr_level)):
                        images_per_row = []
                        for col in range(len(curr_level[0])):
                            images_per_row.append(Image.open(curr_level[row][col]))
                        images.append(images_per_row)
                    level_img = join_images(*images, bg_color='green', alignment=(0.5, 0.5))
                    im_arr = np.asarray(level_img)
                    action = np.argmax(agent.predict(np.array([im_arr]))) + 1
                    # action = 0
                ...
                if info[0]["solved"]:
                    ...
                    to_char_level(final_map,
                                  dir=f'cnn_playable_maps_obs_{obs_size}_ep_len_{ep_len}_{goal_map_size}/{success_count}.txt')
